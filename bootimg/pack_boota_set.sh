#!/usr/bin/env bash
# bootimg/pack_boota_set.sh — pack the boota+AVB triple for slot-A flashing
# on the unlocked-orange C60 (per re/CHANGELOG.md c60-v0.1 recipe).
#
...
#                    on C60 stock u-boot per recipe; missing dtbo descriptor
#                    drops to fastboot)
#
# This is the iteration that "folds the hand-roll into the build pipeline"
# (per c60_mainline_boot_recipe). The recipe was previously in /tmp/.
#
# USAGE
...
# Constant sizes per stock C60 GPT (see profiles/emmc.env / c60-gpt-lbas):
...
# Note: header v0 + --second is the empirically validated combo on this
# u-boot (see c60_mainline_boot_recipe lesson 3). NOT --dtb / header v2.
mkbootimg \
    --kernel "$KERNEL" \
    --second "$DTB" \
...
# but truncating intermediate gives a predictable boot.img layout that
# matches the existing on-device flash and CHANGELOG hashes).
...
# Build the dtbo file inline with Python — keeps the recipe in-tree.
...
# from avb partition data!' and drops to fastboot (recipe lesson 1).
echo "===> [3/3] vbmeta-c60.img (chain boot + dtbo)"
rm -f "$VBMETA_IMG"

# No --padding_size: vbmeta is tiny (~1.5 KiB) and the partition is sized
# by GPT. avbtool make_vbmeta_image doesn't pad by default — that matches
# the on-device 1536-byte vbmeta-c60.img in re/CHANGELOG.
