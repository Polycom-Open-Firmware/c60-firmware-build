...
#
# The primary pipeline does not use this initramfs: `build.sh --profile=emmc`
# produces a kernel with no embedded initramfs and root comes up via partlabel
# against system_a. This script is retained for bring-up debugging — drop a
# kernel into RAM via netboot / SDP recovery / fastboot boot, pass this
# initramfs externally, and get a serial shell on ttymxc1 to inspect the
# running kernel without committing anything to eMMC.
#
# Produces (under initramfs/out/):
#   rootfs/                    populated tree
#   initramfs.cpio.gz          standalone cpio for external-ramdisk use
#                              (the NXP boota external-ramdisk path is broken
#                              on mainline DT, but the cpio works for netboot
#                              or initrd= experiments)
#
# Goal: reach a serial shell on ttymxc1 to prove the kernel boots. DHCP + DSA
# user-port bring-up happens here too (no userspace required) so the first
# boot can be debugged over the LAN as well as serial.
#
# Busybox: pulled from Debian's `busybox-static` package on the host (apt
# install busybox-static). Only the static binary is extracted; no chroot/
# debootstrap dance. Override the path via BUSYBOX=/path/to/busybox-static.
